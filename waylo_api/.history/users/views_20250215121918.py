...
def user_login_view(request):
    ...
    username = request.data.get("username")  # 요청에서 username 가져오기

    if not username:
        return Response({"error": "아이디(username)를 입력하세요."}, status=status.HTTP_400_BAD_REQUEST)

    user_exists = User.objects.filter(username=username).exists()  # 존재 여부 확인

    if user_exists:
        return Response({"message": "사용자가 존재합니다."}, status=status.HTTP_200_OK)
    else:
